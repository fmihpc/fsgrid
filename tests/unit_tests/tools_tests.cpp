...
TEST(FsGridToolsTests, calcLocalStart1) {
   constexpr fsgrid::FsSize_t numGlobalCells = 1024u;
   constexpr fsgrid::Task_t numTasks = 32u;

   ASSERT_EQ(fsgrid::calcLocalStart(numGlobalCells, numTasks, 0), 0);
   ASSERT_EQ(fsgrid::calcLocalStart(numGlobalCells, numTasks, 1), 32);
   ASSERT_EQ(fsgrid::calcLocalStart(numGlobalCells, numTasks, 2), 64);
   ASSERT_EQ(fsgrid::calcLocalStart(numGlobalCells, numTasks, 3), 96);
}

TEST(FsGridToolsTests, calcLocalStart2) {
   constexpr fsgrid::FsSize_t numGlobalCells = 666u;
   constexpr fsgrid::Task_t numTasks = 64u;

   for (int i = 0; i < 26; i++) {
      ASSERT_EQ(fsgrid::calcLocalStart(numGlobalCells, numTasks, i), i * 11);
   }
   for (int i = 26; i < numTasks; i++) {
      ASSERT_EQ(fsgrid::calcLocalStart(numGlobalCells, numTasks, i), i * 10 + 26);
   }
}
...
TEST(FsGridToolsTests, calcLocalSize1) {
   ...
   for (int i = 0; i < numTasks; i++) {
      ASSERT_EQ(fsgrid::calcLocalSize(numGlobalCells, numTasks, i), 32);
   }
}
...
TEST(FsGridToolsTests, calcLocalSize2) {
   ...
   for (int i = 0; i < 26; i++) {
      ASSERT_EQ(fsgrid::calcLocalSize(numGlobalCells, numTasks, i), 11);
   }
...
TEST(FsGridToolsTests, globalIDtoCellCoord1) {
   constexpr std::array<fsgrid::FsSize_t, 3> globalSize = {3, 7, 45};

   for (fsgrid::FsSize_t i = 0; i < globalSize[0] * globalSize[1] * globalSize[2]; i++) {
      const std::array<fsgrid::FsIndex_t, 3> result = {
          (fsgrid::FsIndex_t)(i % globalSize[0]),
          (fsgrid::FsIndex_t)((i / globalSize[0]) % globalSize[1]),
          (fsgrid::FsIndex_t)((i / (globalSize[0] * globalSize[1])) % globalSize[2]),
      };
      ASSERT_EQ(fsgrid::globalIDtoCellCoord(i, globalSize), result);
   }
}
...
TEST(FsGridToolsTests, globalIDtoCellCoord_globalSize_would_overflow) {
   ...
   constexpr std::array<fsgrid::FsSize_t, 3> globalSize = {maxUint, 1, 1};
   const std::array<fsgrid::FsIndex_t, 3> result = {
       -2147483648,
       -2147483648,
       -2147483648,
   };
   ASSERT_EQ(fsgrid::globalIDtoCellCoord(globalSize[0] - 1, globalSize), result);
}
...
TEST(FsGridToolsTests, globalIDtoCellCoord_globalSize_is_maximum_int) {
   ...
   constexpr std::array<fsgrid::FsSize_t, 3> globalSize = {maxInt, maxInt, maxInt};

   std::array<fsgrid::FsIndex_t, 3> result = {maxInt - 1, 0, 0};
   ASSERT_EQ(fsgrid::globalIDtoCellCoord(globalSize[0] - 1, globalSize), result);

   result = {0, 1, 0};
   ASSERT_EQ(fsgrid::globalIDtoCellCoord(globalSize[0], globalSize), result);

   result = {0, 0, 1};
   ASSERT_EQ(fsgrid::globalIDtoCellCoord((int64_t)globalSize[0] * globalSize[0], globalSize), result);

   result = {maxInt - 1, maxInt - 1, 0};
   ASSERT_EQ(fsgrid::globalIDtoCellCoord((int64_t)globalSize[0] * globalSize[0] - 1, globalSize), result);
}
...
Decomposition computeDecomposition(const SystemSize systemSize, const fsgrid::Task_t nProcs) {
   const auto dd = fsgrid::computeDomainDecomposition(
       {
           systemSize.x,
           systemSize.y,
           systemSize.z,
...
TEST(FsGridToolsTests, MPI_err_check_should_throw) {
   EXPECT_THROW(
       {
          try {
             fsgrid::mpiCheck(MPI_SUCCESS + 1, "Should throw with unsuccessful check");
...
TEST(FsGridToolsTests, MPI_err_check_should_pass) { fsgrid::mpiCheck(MPI_SUCCESS, "This should pass"); }
